...
for epoch in range(num_epochs):
    ...
    # Load data
    data = gaussianGridDataset(n=num_grid, n_data=n_data, sig=sigma)
    ...
    if epoch % 2 == 0:
        ...
        if not os.path.exists('out_gaussiangrid2d_model_from_PacGAN_with_metric/'):
            os.makedirs('out_gaussiangrid2d_model_from_PacGAN_with_metric/')
        plt.savefig('out_gaussiangrid2d_model_from_PacGAN_with_metric/{}.png'.format(str(epoch).zfill(3)), bbox_inches='tight')
        plt.close(fig)
        #print('spent time for drawing graph is {}'.format(time.time()-start))
        samples = generator(noise(2500)).cpu().detach().numpy()
        num_modes, num_high_qual_samples, mode_counter = mode_collapse_metric(samples, num_grid, sigma)
        
        print('epoch # :', epoch, 'gamma :', gamma, 'gen loss :', g_error.item())
