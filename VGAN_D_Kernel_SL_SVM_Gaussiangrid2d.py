...
import pyro
from pyro.contrib.gp.kernels.isotropic import RBF as RBFpyro
...
def exp(x, data, variance, gamma):
    ...
    return variance * torch.exp(-gamma*torch.norm(x - data)**2)


def exp2(x, y, data, variance, gamma):
...
def discriminator(x, alpha, rho, support_vecs):
    ...
    for i in range(len(x)):
        for j in range(len(support_vecs)):
            D[i] += alpha[j] * exp(x[i], support_vecs[j], variance, gamma)
...
def discriminator2(x, y, alpha, rho, support_vecs):
    ...
    D = 0
    for j in range(len(support_vecs)):
        D += alpha[j] * exp2(x, y, support_vecs[j], variance, gamma)
...
def train_generator(optimizer, fake_data, alpha, rho, support_vecs):
    ...
    prediction = discriminator2(fake_data[:, 0], fake_data[:, 1], alpha, rho, support_vecs)
    ...
    for n_batch, (real_batch) in enumerate(tqdm(data_loader)):
        ...
        if n_batch % 5 == 0:
            if n_batch == 0 and epoch == 0:
                real_g_data = data.data[:5*len(real_batch)]
            ...
            #start = time.time()
            alpha, rho, support_vecs = SVM_with_kernel(real_g_data, fake_g_data)
            #print('spent time for getting SVM is {}'.format(time.time()-start))
            real_g_data = torch.zeros(5*len(real_batch), len(real_batch[0]))
